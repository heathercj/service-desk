...
/**
 * A department key's shape, not its membership: departments are created
 * and renamed by administrators at runtime (Section: self-service
 * departments), so there's no fixed list to validate membership against
 * here any more -- that's `requireActiveDepartment`'s job, one layer
 * deeper, against real Department rows. This still enforces shape, since a
 * department key doubles as a URL segment (`/queue/[key]`) and a
 * knowledge-base folder name (`departmentKeyToFolder`).
 */
export const departmentKeySchema = z
  .string()
  .regex(/^[A-Z0-9_]+$/, "Must be uppercase letters, digits, and underscores only")
  .max(80);
